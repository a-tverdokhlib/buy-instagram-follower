...
import { setPackageFAQs } from '@/redux/reducers/admin/packageFAQs'
...
const MyComponentRenderFn: ForwardRefRenderFunction<any, Props> = (
  ...
  const { packageFAQs } = useAppSelector((state) => state.adminPackageFAQ)
  const [collapse, setCollapse] = useState(false)
  const refClearCheckedList = useRef<HTMLDivElement>(null)
  useEffect(() => {
    getPackageFAQs()
  }, [])
  const getPackageFAQs = async () => {
    const resp = await packageFAQService.search(
      `categoryId=${props.category._id}`,
    )
    dispatch(setPackageFAQs(resp))
  }
    ...
    <>
      ...
      {!collapse ? (
        <PackageFAQList
          ...
          packageFAQs={packageFAQs[`${props.category._id}`] || []}
...
export const PackageFAQBlock = React.forwardRef(MyComponentRenderFn)
