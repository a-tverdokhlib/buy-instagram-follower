...
const MyComponentRenderFn: ForwardRefRenderFunction<any, Props> = (
  ...
  const onAllCheckClicked = (e) => {
    if (checkedList.length === 0) {
      setCheckedList([
        ...checkedList,
        ...props.autoLikePacks.map((item: { _id: any }) => item._id),
      ])
    } else if (checkedList.length === props.autoLikePacks.length) {
      setCheckedList([])
    } else {
      setCheckedList([])
    }
        ...
        <div className="py-0 inline-block min-w-full">
          <div className="overflow-hidden">
            <table className="min-w-full text-center border-collapse border border-slate-400">
                ...
                <tr>
                  ...
                  >
                    <input
                      ...
                      type="checkbox"
                      checked={
                        props.autoLikePacks.length === checkedList.length
                          ? true
                          : false
                      }
              ...
              <tbody>
                {props.autoLikePacks.map((autoLikePack, id) => {
                  return (
                    <tr
                      key={autoLikePack._id}
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap text-base font-medium text-gray-900 border border-slate-300">
                        <input
                          ...
                          type="checkbox"
                          checked={
                            checkedList.indexOf(autoLikePack._id) !== -1
                              ? true
                              : false
                          }
                          onChange={(e) => onCheckClicked(e, autoLikePack)}
                        />
                      </td>
                      ...
                      <td className="text-base text-gray-700 px-1 py-4 border border-slate-300">
                        {autoLikePack.coupanDiscount}
                      </td>
                      <td className="text-base text-gray-700 px-1 py-4 border border-slate-300">
                        {moment
                          .utc(autoLikePack.updatedAt)
                          .local()
                      ...
                      <td className="text-base text-gray-700 px-1 py-4 whitespace-nowrap border border-slate-300">
                        <Switch
                          height={25}
                          width={50}
                          onChange={(e) => onSwitchChange(e, autoLikePack)}
                          checked={autoLikePack.isActive}
                        />
                      </td>
                      <td className="text-base text-gray-900 whitespace-nowrap border border-slate-300 px-3">
                        <div className="flex flex-row flex-nowrap w-full justify-center items-center">
                          <span
                            onClick={() => onEditClick(autoLikePack)}
                          ...
                          <span
                            onClick={() => onRemoveClick(autoLikePack)}
...
export const AutoLikePackList = React.forwardRef(MyComponentRenderFn)
